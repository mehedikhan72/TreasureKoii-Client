...
const Dashboard: React.FC = () => {
  ...
  const addOrganizers = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log(error);
	  toast.error("No users found with the given email(s).");
    } finally {
      setLoading(false);
    }
  };
      ...
      {!userAnOrganizer && userDataLoaded && (
        <div className="flex flex-col justify-center items-center">
          <p className="text-2 stroked-text-sm">
            You are not an organizer of this hunt.
          </p>
        ...
        <div>
          ...
          <div className="flex flex-col justify-center items-center">
            <div className="flex justify-center items-center flex-wrap ">
              <Link
                to={{ pathname: `/${slug}/create-puzzle` }}
                className="block"
              >
                <button className="my-btn-1">Create New Puzzle</button>
              </Link>
              <Link
                to={{ pathname: `/${slug}/manage-teams` }}
                className="block"
              >
                <button className="my-btn-1 max-sm:w-max px-4">
                  Manage Teams & Puzzles
                </button>
              </Link>
            </div>
