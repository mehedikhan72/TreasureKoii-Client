		...
		return (
			<div className={props.className}>
				...
				<div>
					...
					{(!!days || !!hours || !!minutes || !!seconds) && `${zeroPad(seconds, 2)} Secs`}
				</div>
			</div>
		);
	}
			...
			{hunt && (
				<>
					<div className="flex flex-col justify-center items-center mt-20 mb-10 mx-10">
						...
						<div className="text-1 my-2 stroked-text-sm">{`${new Date(hunt.start_date).toLocaleString(undefined, {
							weekday: "short",
							year: "numeric",
							month: "short",
							day: "numeric",
						...
						<img src={hunt.poster_img} alt="Hunt" className="w-4/5 max-h-96 object-contain my-5" />
						...
						<div className="styled-div-1 w-full text-center max-w-[40rem]">{hunt.description}</div>
					</div>
					...
					{rules?.length !== 0 && (
						<div>
							<div className="text-1 my-5 stroked-text-sm">
								<p className="text-3">We expect that you follow a set a of rules.</p>
