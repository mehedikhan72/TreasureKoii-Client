...
const PuzzleOrder = () => {
  ...
  const [updatePuzzleOrderMessage, setUpdatePuzzleOrderMessage] =
    useState<string>("");
  const [message, setMessage] = useState<string | null>(null);

  const [loading, setLoading] = useState<boolean>(false);
  const [organizerCheckLoading, setOrganizerCheckLoading] =
  ...
  const puzzleOrderUpdated = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(
        `${slug}/${teamId}/create-puzzle-order/`,
        {
          list: list,
        }
      );
      ...
      console.log(error);
      setMessage(
        "Invalid Puzzle Order. Please check the puzzle IDs and try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      {(loading || organizerCheckLoading || teamsLoading || puzzlesLoading) && (
        <Loading />
      )}
      ...
      {userAnOrganizer && userDataLoaded && (
        <div>
          {allTeamsData.length === 0 && (
            <div>
              <p className="text-2">
                No teams registered for this hunt... yet. You can set puzzle
                order for teams once they register.
              </p>
            </div>
          )}
          {allTeamsData.length > 0 && (
              ...
              <div>
                ...
                <form onSubmit={puzzleOrderUpdated}>
                  <div className="flex flex-col justify-center items-center">
                    ...
                    <p className="text-2 text-green-500">
